...
class YOLOv8Heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        self.method = eval(method)(self.model, self.target_layers)
        self.method.activations_and_grads = ActivationsAndGradients(
            self.model, self.target_layers, None)

        self.colors = np.random.uniform(0, 255, size=(
            len(self.model_names), 3)).astype(np.int)
        ...
        if self.show_box:
            for data in pred:
                ...
                cam_image = self.draw_detections(
                    data[:4], color, class_name, cam_image)
...
if __name__ == '__main__':
    model = YOLOv8Heatmap(**get_params())
    model(imgPath, 'result')
